...
def ESRA(CSZ, Fn, TLn, Pcoef): #function [GHI_ESRA, DNI, DHI]= ESRA(COSZ, doy, TL, pcoef, bis)
    
    TLv = TLn/0.8662 #pasa al TL viejo!!!! 
    ...
    DNI= I0*Fn*exp(-0.8662*TLv*M*deltaR)
    DNI[DNI<0]=0 #cereo por si acaso

    #   IRRADIACION DIFUSA
        
    #funcion de transmision difusa en el zenit (gama=90 grados)
    Trd = -1.5843e-2 + 3.0543e-2*TLv+ 3.797e-4*TLv**2

    #dfuncion angular difusa Fd, para eso defino A0, A1 y A2 (dependen de TL)
    
    A0 = 2.6463e-1 - 6.1581e-2*TLv + 3.1408e-3*TLv**2
    msk_A0 = (A0*Trd)<2e-3
    A0[msk_A0]= 2e-3/Trd[msk_A0]
    
    A1 = 2.0402 + 1.8945e-2*TLv - 1.1161e-2*TLv**2
    A2 =-1.3025 + 3.9231e-2*TLv + 8.5079e-3*TLv**2
...
def OpticalMass(z, proc): #z angulo cenital en GRADOS, proc-> proceso ('Ray', 'O3', 'NO2', 'Mix', 'WV', 'Aer', 'Kas')
    ...
    return m
#tipo 1 devuelve las 3 componentes, tipo o devuelve también luminiscencia y par

def REST2(z, Pcoef, beta, alfa1, alfa2, uo, un, w, rog1, rog2, tipo=1):
    n=size(z)
    cos_z = cos (z*pi/180)
    # defino constantes

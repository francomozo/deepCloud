...
#   ojo!!!, funciona solo para inputs escalares
    ...
    try:
        ...
        for k in range(month-1):
            ...
            doy = n_acum + day
        #return doy1
    #OJO!! lo que viene acá después parece tener problema
        DOY=doy
    except TypeError: #si es un array, hago un for
        DOY=np.ones(np.shape(month))*np.nan #esto puede demorar, es un doble for
        #print(doy)
        for j in range(len(month)):
            n_acum = 0
            for k in range(month[j]-1):
                n_acum = n_acum + dias_del_mes(k+1, bis[j]) 
            DOY[j] = n_acum + day[j]            #print(j)
            #print(mth_day_to_doy(month[j], day[j], bis[j]))
            #doy[j]=mth_day_to_doy(month[j], day[j], bis[j])

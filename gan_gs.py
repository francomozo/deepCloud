...
for index, (LEARNING_RATE, LAMBDA_GP, critic_iter, optim_type, load_dict) in enumerate(hparams):
    ...
    # tb
    writer_gt = SummaryWriter(f"runs/gan/lr_{LEARNING_RATE}-lambda_{LAMBDA_GP}-critic_iter_{critic_iter}-optim_type_{optim_type}-load_dict_{load_dict}/gt")
    writer_pred = SummaryWriter(f"runs/gan/lr_{LEARNING_RATE}-lambda_{LAMBDA_GP}-critic_iter_{critic_iter}-optim_type_{optim_type}-load_dict_{load_dict}/pred")
    step = 0

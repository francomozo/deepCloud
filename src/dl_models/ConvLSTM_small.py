...
class CLSTM_cell(nn.Module):
    ...
    def forward(self, inputs=None, hidden_state=None):
        ...
        for index in range(self.seq_len):
            ...
            print(f'index {index}')
            print(f'CLSTM cell input shape: {x.shape}')
            print(f'CLSTM cell hidden state shape: {hx.shape}')
...
def convlstm_encoder_params(in_chan=7, seq_len=4, img_size=(128,128)):
    ...
    convlstm_encoder_params = [
        ...
        [
            CLSTM_cell(shape=(img_size[0], img_size[1]), input_channels=32, filter_size=5, num_features=64, seq_len=seq_len),
            CLSTM_cell(shape=(img_size[0]//2, img_size[1]//2), input_channels=64, filter_size=5, num_features=96, seq_len=seq_len),
            CLSTM_cell(shape=(img_size[0]//4, img_size[1]//4), input_channels=96, filter_size=5, num_features=128, seq_len=seq_len)
        ]
    ]
    return convlstm_encoder_params

def convlstm_decoder_params(seq_len, img_size=(128,128)):
    convlstm_decoder_params = [
        ...
        [
            ...
            CLSTM_cell(shape=(img_size[0], img_size[1]), input_channels=96, filter_size=5, num_features=64, seq_len=seq_len),
        ]
    ]
...
class ConvLSTM(nn.Module):
    ...
    def __init__(self, seq_len, in_chan=7, input_seq_len=2, img_size=(128,128)):
        super(ConvLSTM, self).__init__()

        encoder_params = convlstm_encoder_params(in_chan, input_seq_len, img_size=img_size)
        decoder_params = convlstm_decoder_params(seq_len, img_size=img_size)
    
        self.encoder = Encoder(subnets=encoder_params[0], rnns=encoder_params[1])
    ...
    def forward(self, input, future_seq=10):
        ...
        print('Decoder')
        # print(f'Decoder input shape {len(state)}')

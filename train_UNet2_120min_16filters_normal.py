...
for lr, mdl in grid_search:
  ...
  MODEL_PATH = 'checkpoints/R3/120min/'
  ...
  checkpoint_folder = 'R3/120min/'
  model_name = f'120min_UNET{mdl}_{dataset}_{train_loss}_filters{init_filters}_{output_activation}_diff{predict_diff}_retrain{retrain}'
  
  comment = f' batch_size:{batch_size} lr:{lr} model:{mdl} train_loss:{train_loss} predict_diff{predict_diff}'
  
  writer = SummaryWriter(log_dir='runs/predict_120min/'+model_name, comment=comment)
  #writer = None
  print(model_name)

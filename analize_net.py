...
with torch.no_grad():
    for val_batch_idx, (in_frames, out_frames, out_time) in enumerate(val_loader):
        ...
        if MAE_loss.detach().item() > worst_MAE_error:
            ...
            worst_MAE_images[0:3] = in_frames[0]
            worst_MAE_images[3] = out_frames[0, 0]
            worst_MAE_images[4] = frames_pred[0, 0]
            
        if MAE_loss.item() < best_MAE_error:
            best_MAE_error = MAE_loss.item()
            best_MAE_time = out_time[0, FRAME_OUT]
            best_MAE_images[0:3] = in_frames[0]
            best_MAE_images[3] = out_frames[0,0]
            best_MAE_images[4] = frames_pred[0,0]
        ...
        if MSE_loss.detach().item() > worst_MSE_error:
            ...
            worst_MSE_images[0:3] = in_frames[0]
            worst_MSE_images[3] = out_frames[0, 0]
            worst_MSE_images[4] = frames_pred[0, 0]
        if MSE_loss.item() < best_MSE_error:
            best_MSE_error = MSE_loss.detach().item()
            best_MSE_time = out_time[0, FRAME_OUT]
            best_MSE_images[0:3] = in_frames[0]
            best_MSE_images[3] = out_frames[0,0]
            best_MSE_images[4] = frames_pred[0, 0]
        ...
        if PSNR_per_hour[hour][-1] < worst_PSNR_error:
            ...
            worst_PSNR_images[0:3] = in_frames[0]
            worst_PSNR_images[3] = out_frames[0,0]
            worst_PSNR_images[4] = frames_pred[0,0]
        if PSNR_per_hour[hour][-1] > best_PSNR_error:
            best_PSNR_error = PSNR_per_hour[hour][-1]
            best_PSNR_time = out_time[0, FRAME_OUT]
            best_PSNR_images[0:3] = in_frames[0]
            best_PSNR_images[3] = out_frames[0,0]
            best_PSNR_images[4] = frames_pred[0,0]
        ...
        if SSIM_loss.detach().item() < worst_SSIM_error:
            ...
            worst_SSIM_images[0:3] = in_frames[0]
            worst_SSIM_images[3] = out_frames[0,0]
            worst_SSIM_images[4] = frames_pred[0,0]
        if SSIM_loss.item() > best_SSIM_error:
            best_SSIM_error = SSIM_loss.detach().item()
            best_SSIM_time = out_time[0, FRAME_OUT]
            best_SSIM_images[0:3] = in_frames[0]
            best_SSIM_images[3] = out_frames[0,0]
            best_SSIM_images[4] = frames_pred[0,0]

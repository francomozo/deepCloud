...
from src.lib.utils import save_checkpoint
...
CSV_PATH_VAL = '/clusteruy/home03/DeepCloud/deepCloud/data/mvd/val_cosangs_in3_out1.csv' # this file doesnt exist, need to generate it
...
val_mvd = MontevideoFoldersDataset(path=DATA_PATH_VAL, in_channel=3, out_channel=1,
                                   min_time_diff=5, max_time_diff=15,
...
for epoch in range(NUM_EPOCHS):
    ...
    with torch.no_grad():
        ...
        for val_batch_idx, (in_frames, out_frames) in enumerate(val_loader):
            ...
            frames_pred = gen(in_frames)
            val_loss = criterion(frames_pred, out_frames)

            val_epoch_loss.append(val_loss.detach().item())    
        val_loss = sum(val_epoch_loss)/len(val_epoch_loss)

    
    # print losses (gen, disc, val_loss)
    print(f'\t -> Gen_epoch_loss: {gen_loss_by_epochs[-1]:.8f}, Disc_epoch_loss: {disc_loss_by_epochs[-1]:.8f}. Val_loss: {val_loss:.8f}.')
